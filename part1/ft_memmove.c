...
**	Safely copies memory, handling overlapping regions by copying in reverse (end-to-start) when 'dest' > 'src'.
**	Pointer Casting: Casts 'dest' and 'src' to char * for byte-wise operations.
**	Reverse Copy: Uses a backward loop to prevent overwriting source data if regions overlap.
